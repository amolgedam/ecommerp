...
class Model_hsn extends CI_Model
{
    ...
    public function create($data = array())
	{
		if($data) {
			...
			$create = $this->db->insert('wo_hsn', $data);
	...
	public function fetchDataByNameCode($name='', $code='')
	{
	    if($_SESSION['wo_role'] == 'superadmin'){
    	    $query = $this->db->select('*')
    							->from('wo_hsn')
    							->where('hsn_name', $name)
    							->or_where('hsn_code', $code)
    							// ->where(['company_id' => $this->session->userdata['wo_company']])
    							->get();
	...
	public function update($data = array())
	{
		if($data) {
			...
			$update = $this->db->update('wo_hsn', $data);
	...
	{
		$this->db->where('id', $id);
		return $result=$this->db->delete('wo_hsn');
	}
