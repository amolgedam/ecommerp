<!--< ?php echo "<pre>"; print_r($lastData); exit; ?>-->
                  ...
                  success:function(gstResponse){
                      ...
                      $.ajax({            
                          ...
                          success:function(discountResponse){
                              ...
                              $.ajax({   
                                    url: base_url + 'sales_order/getOrderData/',
                                    type: 'post',
                                    dataType: 'json',
                                    ...
                                    success:function(response){
                                        ...
                                        $.each(response, function(index, value){
                                            ...
                                            if(value.item_status == 'soldout')
                                            {
                                                alert('Product not Available or Soldout');
                                            }
                                            else
                                            {
                                              // alert(value.item_status);

                                                var mrp = parseFloat(value.mrp);
                                            
                                                var cgst = parseFloat(value.cgst);
                                                var sgst = parseFloat(value.sgst);
                                                var igst = parseFloat(value.igst);
                                                var totgst = cgst + sgst + igst;
                                                ...
                                                html += '<td>';
                                                    ...
                                                    html += '<input type="hidden" name="pno[]" value="'+value.barcodeid+'" required readonly>';

                                                    html += '<input type="text" name="pnoname[]" value="'+value.barcode+'" id="pno_'+row_id+'" required readonly>';
                                                html += '</td>';
                                                ...
                                                {
                                                    read = 'readonly';
                                                    mrp = '<input type="text" name="oldbaseprice[]" value="'+value.mrp+'" onchange="mrpChange('+row_id+')" id="baseprice_'+row_id+'" >';
                                                }
                                                else
                                                {
                                                    read = '';
                                                    mrp = '<input type="text" name="oldbaseprice[]" value="'+value.mrp+'" onchange="mrpChange1('+row_id+')" id="baseprice_'+row_id+'" >';
                                                }
                                                ...
                                                html += '<td>';
                                                    html += '<input type="text" name="finalprice[]" id="finalprice_'+row_id+'" class="finalClass" value="'+value.mrp+'" readonly>';
                                                    html += '<input type="hidden" name="hfinalprice[]" id="hfinalprice_'+row_id+'" value="'+value.mrp+'" readonly>';
                                                    
                                                    html += '<input type="hidden" name="hfinalprice1[]" id="hfinalprice1_'+row_id+'" value="'+value.mrp+'" readonly>';
                                                html += '</td>';
                                                
                                                html += '<td>';
                                                    html += '<input type="hidden" name="sku[]" value="'+value.sku_id+'" id="sku_'+row_id+'" readonly>';

                                                    html += '<input type="text" name="sku_code[]" value="'+value.sku_code+'" id="sku_'+row_id+'" readonly>';
                                                html += '</td>';
