...
class Purchase_invoiceitem extends Admin_Controller 
{
    ...
    public function deleteItem()
    {
        ...
        $gstData = $this->model_gst->fetchAllDataByID($orderData['gst_id']);

        $totgst = $gstData['cgst'] + $gstData['sgst'] + $gstData['igst'];

        $newTax = $orderData['base_price'] * $totgst / 100;

        $newGst = $order['total_tax'] - $newTax;

        $newGamt = $order['gamt'] - $newGst;

        $invoice_value = $newGamt + $newGst;
