...
class Opening_stockitem extends Admin_Controller 
{
	...
	public function create()
	{
	    ...
	    if ($this->form_validation->run() == TRUE) {
            ...
            $value = $qty * $this->input->post('base_price');

            $invoice_value = $orderData['total_invoice'] + $value;
	...
	public function update()
	{
	    ...
	    if ($this->form_validation->run() == TRUE) {
            ...
            $gstData = $this->model_gst->fetchAllDataByID($this->input->post('gst'));
            $totgst = $gstData['sgst'] + $gstData['cgst'] + $gstData['igst'];
            $total_tax = $gamout * $totgst / 100;
            
            $invoice_value = $oldinvoice_value + $gamout + $total_tax;            
    ...
    public function deleteItem()
    {
        ...
        $invoice_value = $order['tot_invoicevalue'] - $orderData['wspp'];
