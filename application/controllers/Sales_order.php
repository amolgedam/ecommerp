...
class Sales_order extends Admin_Controller 
{
        ...
        else
        {
    	   ...
    	   // $result = array();
            foreach($data as $key => $value)
            {
                ...
                $salesOrderQty = $this->model_salesorder->fecthQtyData($value['id']);
    ...
    public function jobInvoice()
    {
        ...
        else
        {
            ...
            $this->data['salesinvoicetype'] = 'invoice';
            ...
            $this->data['discount'] = $this->model_discount->fecthAllData();
            $this->data['gstData1'] = $this->model_gst->fecthAllData();
            ...
            $salesorderData = $this->model_salesorder->fetchQtyDataById($id);
            $this->data['salesorderqty'] = $salesorderData;

            $this->data['allData'] = $this->model_salesorder->fecthAllDatabyID($salesorderData['salesorder_id']);

            $data = array(
                            'purchase_id' => $salesorderData['jobsheet_id'],
                            'purchase_type' => 'production'
                        );

            $this->data['barcodeData'] = $this->model_barcode->getBarcodeDataByPurchaseId($data);

            $this->render_template('admin_view/salesMaster/salesOrder/createInvoice', $this->data); 
        }
    }
    ...
    public function createInvoice()
    {
        ...
        else
        {
            ...
            // echo "<pre>"; print_r($salesorderData);
    ...
    public function getOrderData()
    {
        ...
        $orderData = $this->model_salesorder->fecthAllDatabyID($orderid);

        $orderQtyData = $this->model_salesorder->fecthQtyData($orderid);

        foreach ($orderQtyData as $key => $value) {
            
            $data = array(
                            'purchase_id' => $value->jobsheet_id,
                            'purchase_type' => 'production'
                        );

            $barcodeData = $this->model_barcode->getBarcodeDataByPurchaseId($data);

            // $data1 = $data2 = $data3 = array();

            foreach ($barcodeData as $key => $valueb) {
                
                $barcodeData = $this->model_barcode->fetchAllDataByBarcode($valueb['barcode']);
                $skuData = $this->model_sku->fecthSkuDataByID($barcodeData['sku_code']);
                $gstData = $this->model_gst->fetchAllDataByID($barcodeData['gst_id']);
                ...
                $result[] = array(
                                                ...
                                                'basic_rate' => $barcodeData['barcode'],
                                                'pur_netprice' => $barcodeData['pur_netprice'],
                                                'mrp' => $barcodeData['mrp'],
                                                'wsp' => $barcodeData['wsp'],
                                                'qty' => $barcodeData['qty'],
                                                'purchase_type' => $barcodeData['purchase_type'],
                                                'item_status' => $barcodeData['item_status'],
                                                'gst_id' => $barcodeData['gst_id'],
                                                'unit_id' => $barcodeData['unit_id'],
                                                'hsn' => $barcodeData['hsn'],
                                                'salesmancomm' => $barcodeData['salesmancomm'],
                                                'discountcode' => $barcodeData['discountcode'],
                                                'brand_id' => $barcodeData['brand_id'],
                                                'sku_id' => $skuData['id'],
